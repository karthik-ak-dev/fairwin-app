...
/**
 * DEPRECATED: Submit winner payout transaction
 *
 * Payouts now happen AUTOMATICALLY on-chain when Chainlink VRF callback completes.
 * The contract selects winners and pays them in a single atomic transaction.
 * This function is kept for backward compatibility but should NOT be used.
 *
 * Winners are paid automatically in the VRF fulfillRandomWords callback.
 * Backend only needs to listen to WinnersSelected event to record who won.
 *
 * @deprecated Use event listeners instead - payouts are automatic
 */
export async function submitPayout(
  ...
  throw new Error(
    'submitPayout is deprecated. Payouts happen automatically on-chain via VRF callback. ' +
    'Listen to WinnersSelected event instead.'
  );
}
